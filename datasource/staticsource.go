...
func NewStaticSource(name string, cols []string, msgs []Message) *StaticSource {
	return &StaticSource{
		table: name,
		cols:  cols,
		vals:  msgs,
...
func (m *StaticSource) CreateIterator(filter expr.Node) Iterator { return m }
func (m *StaticSource) MesgChan(filter expr.Node) <-chan Message {
	iter := m.CreateIterator(filter)
	return SourceIterChannel(iter, filter, m.exit)
}

func (m *StaticSource) Next() Message {
