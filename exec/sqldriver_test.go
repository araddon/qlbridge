...
func init() {
	...
	mockcsv.MockData["users_db"] = `user_id,email,interests,reg_date,item_count
...
hT2impsabc345c,"not_an_email","swimming","2009-12-11T19:53:31.547Z",12`
	...
	mockcsv.MockData["orders_db"] = `user_id,item_id,price,order_date,item_count
...
func TestSql1(t *testing.T) {
	...
	sqlText := `
		select 
	        user_id, email, item_count * 2, yy(reg_date) > 10 AS reg_date_gt10
	    FROM users
	    WHERE 
	    	yy(reg_date) > ? 
    `
